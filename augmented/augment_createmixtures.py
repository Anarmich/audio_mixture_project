...
def main(data_dir, output_json, num_combinations, num_augmentations=5):
    source_dirs = {
    ...
    # Limit the number of combinations to the specified limit
    ...
    for i, (road_file, wind_file, powertrain_file) in enumerate(combinations):
        ...
        # Check if output file path is too long
        if len(output_file) >= 255:
            print(f"Skipping {output_file} due to file path length.")
            continue
...
if __name__ == "__main__":
    ...
    num_combinations = 50  # Set the desired number of mixtures to create
    for data_dir in data_dirs:
        output_json = os.path.join(data_dir, 'data_manifest.json')
        main(data_dir, output_json, num_combinations)
