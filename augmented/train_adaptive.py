...
class AdaptiveLoss(nn.Module):
    ...
    def forward(self, predictions, targets):
        road_pred, wind_pred, powertrain_pred = predictions
        road_target, wind_target, powertrain_target = targets
        
        road_loss = nn.functional.mse_loss(road_pred, road_target, reduction='sum')
        wind_loss = nn.functional.mse_loss(wind_pred, wind_target, reduction='sum')
        powertrain_loss = nn.functional.mse_loss(powertrain_pred, powertrain_target, reduction='sum')
...
if __name__ == "__main__":
    ...
    train_model(model, train_loader, val_loader, num_epochs=20, learning_rate=0.0001, device='cuda' if torch.cuda.is_available() else 'cpu')
