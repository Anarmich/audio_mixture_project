...
def prepare_input_spectrogram(spectrogram_path):
    ...
    normalized_spectrogram, mean, std = normalize_spectrogram(spectrogram)
    normalized_spectrogram = torch.tensor(normalized_spectrogram, dtype=torch.float32).unsqueeze(0).unsqueeze(0)  # Add batch and channel dimensions
    return normalized_spectrogram, mean, std
...
if __name__ == "__main__":
    ...
    input_spectrogram, mean, std = prepare_input_spectrogram(input_spectrogram_path)

    # Predict separation
    road_output, wind_output, powertrain_output = predict_separation(model, input_spectrogram, device)

    # Denormalize outputs
    road_output = denormalize_spectrogram(road_output, mean, std)
    wind_output = denormalize_spectrogram(wind_output, mean, std)
    powertrain_output = denormalize_spectrogram(powertrain_output, mean, std)
